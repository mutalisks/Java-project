...
build:function(){
   ...
   //取得页面的宽度和高度
        const sClientWidth = document.body.clientWidth;
        const sClientHeight = document.body.clientHeight;
        const sScrollTop = document.body.scrollTop;
        //
        let sleft = (sClientWidth / 2) - (oDialog.offsetWidth / 2);
        const iTop = -80 + (sClientHeight / 2 + sScrollTop) - (oDialog.offsetHeight / 2);
        let sTop = iTop > 0 ? iTop : (sClientHeight / 2 + sScrollTop) - (oDialog.offsetHeight / 2);
   ...
   this.showSome()},hiddenSome:function(){let i;
        var tag=this.info.someHiddenTag.split(",");
   if(tag.length==1&&tag[0]=="")tag.length=0;
   for(i = 0; i<tag.length; i++){this.hiddenTag(tag[i])};
   var ids=this.info.someHiddenEle.split(",");
   if(ids.length==1&&ids[0]=="")ids.length=0;
   for(i = 0; i<ids.length; i++){this.hiddenEle(ids[i])}
   var ids=this.info.someDisabledBtn.split(",");
   if(ids.length===1&&ids[0]==="")ids.length=0;
   for(i = 0; i<ids.length; i++){this.disabledBtn(ids[i])}
