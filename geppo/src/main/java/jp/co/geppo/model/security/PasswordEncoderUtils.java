...
class PasswordEncoderUtils {
     ...
     * @param actual actual
     * @return expectedとactualが一致する場合true
     */
    static boolean equals(String expected, String actual) {
        final byte[] expectedBytes = bytesUtf8(expected);
        final byte[] actualBytes = bytesUtf8(actual);
        final int expectedLength = expectedBytes == null ? -1 : expectedBytes.length;
        final int actualLength = actualBytes == null ? -1 : actualBytes.length;

        int result = expectedLength == actualLength ? 0 : 1;
        for (int i = 0; i < actualLength; i++) {
            final byte expectedByte = expectedLength <= 0 ? 0 : expectedBytes[i % expectedLength];
            final byte actualByte = actualBytes[i % actualLength];
